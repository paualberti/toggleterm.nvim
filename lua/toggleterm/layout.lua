...
---@return integer
M.get_editor_width = function() return vim.o.columns end
...
local function calculate_dim(desired_size, exact_size, min_size, max_size, total_size)
  ...
  if max_val then ret = math.min(ret, max_val) end
  if min_val then ret = math.max(ret, min_val) end
...
---@class (exact) ToggleTerm.WinLayout
---@field width integer
...
M.get_fullscreen_win_opts = function()
  ...
  local width = total_width - 2 * config.float_opts.padding
  if config.float_opts.border ~= "none" then
    width = width - 2 -- The border consumes 1 col on each side
  end
  if config.float_opts.max_width > 0 then
    local max_width = math.floor(calc_float(config.float_opts.max_width, total_width))
    width = math.min(width, max_width)
  end
  local height = total_height - 2 * config.float_opts.padding
  if config.float_opts.max_height > 0 then
    local max_height = math.floor(calc_float(config.float_opts.max_height, total_height))
...
---@param gap integer
---@return ToggleTerm.WinLayout root_dim New dimensions of the original window
---@return ToggleTerm.WinLayout new_dim New dimensions of the new window
M.split_window = function(winid, direction, gap)
  if direction == "auto" then direction = vim.o.splitright and "right" or "left" end

  local float_config = vim.api.nvim_win_get_config(winid)
  ---@type ToggleTerm.WinLayout
  local dim_root = {
